...
/// Runs the test and file inputs for a specific day's puzzle solver.
///
/// - Parameter solver: An instance conforming to `DaySolver`.
public func runDay<S: DaySolver>(solver: S, resourceBundler: Bundle) {
...
/// Reads a resource file from the bundle (preferred for SPM) or potentially filesystem.
///
/// - Parameters:
///   - resourceName: The base name of the resource (e.g., "day-01").
///   - extension: The file extension (defaults to "txt").
/// - Returns: The content of the file as a string, or nil if not found or unreadable.
private func readResource(named resourceName: String, resourceBundler: Bundle, extension ext: String = "txt") -> String? {
    ...
    guard let url = resourceBundler.url(forResource: resourceName, withExtension: ext) else {
        print("Error: Resource file '\(resourceName).\(ext)' not found in Swift Package bundle.")
        return nil
    }
    do {
    ...
    print("Info: Not running as Swift Package. Attempting to read '\(resourceName).\(ext)' from current directory.")
